...
def importVideo(videoPath):
    print("Please be patient! The video is being indexed. The GUI will pop up once this has completed.")
    # Uses numpy slicing to access frames
    # Grab a frame: vid[0] = frame 0
    cap = pims.PyAVReaderIndexed(videoPath) 
    
    print("Thanks for waiting! Indexing complete. Look for the Tkinter tab.")
...
def saveVideosUsingFrames(quadrantNumber, voleEntryList, fps, cap, HEIGHT, WIDTH, DATE_OF_VIDEO, master, SAVED_VIDEO_PATH, progressBar):
    ...
    for voleEntry in voleEntryList:
        ...
        if quadrantNumber == "Q1":
            ...
            img = cap[startingFrame][HEIGHT:(2*HEIGHT), WIDTH:(2*WIDTH)]

        elif quadrantNumber == "Q2":
            # Q2: Bottom Left Video Quadrant
            img = cap[startingFrame][HEIGHT:(2*HEIGHT), :WIDTH]

        elif quadrantNumber == "Q3":
            # Q3: Top Left Video Quadrant
            img = cap[startingFrame][:HEIGHT, :WIDTH]

        elif quadrantNumber == "Q4":
            # Q4: Top Right Video Quadrant
            img = cap[startingFrame][:HEIGHT, WIDTH:(2*WIDTH)]
        ...
        # Append MetaData to the filename that will be written out. Specified as Today's Date(MM-DD-YYYY), File Created Date(MM-DD-YYYY), Time Created Today(HH-MM-SS), Vole Number(#).
        splitDateAttributes = DATE_OF_VIDEO.split()
        time = datetime.today()
        todaysDate = time.strftime('%b-%d-%Y')
        timeCreated = time.strftime('%H:%M:%S').replace(':', "-")
        saveVideoPathPlusMetadata = SAVED_VIDEO_PATH + ("%s_%s-%s-%s_%s_%s.mp4" % (todaysDate, splitDateAttributes[1], splitDateAttributes[2], splitDateAttributes[4],  
                                                                                        timeCreated, voleEntry.getVoleNumber()))
        ...
        for i, fc in enumerate(cap[startingFrame:endingFrame]):
            # Crop the video based on the quadrant number.
            if quadrantNumber == "Q1":
                # Q1: Bottom Right Video Quadrant
                fc = fc[HEIGHT:(2*HEIGHT), WIDTH:(2*WIDTH)]

            elif quadrantNumber == "Q2":
                # Q2: Bottom Left Video Quadrant
                fc = fc[HEIGHT:(2*HEIGHT), :WIDTH]
                ...
                # Q4: Top Right Video Quadrant
                fc = fc[:HEIGHT, WIDTH:(2*WIDTH)]

            # If the user selected a ROI, crop the frame to the selected ROI
            if r != (0, 0, 0, 0):
                # Crop image to ROI
                fc = fc[int(r[1]):int(r[1]+r[3]), int(r[0]):int(r[0]+r[2])]

            # Write the newly cropped frame out to disk
            writer.write(fc)

            # Increment progress bar     
            progressBar['value'] = (i / float(endingFrame) * 100.0)
            master.update()
...
def main():
    ...
    VIDEO_PATH = r'd:\Donaldson Lab\Current Work\Video Split Gui\short.mp4'
    ...
    DATE_OF_VIDEO = ""
    # split the video horizontally. The addition statement accounts for blackspace. Changing this number moves the quadrant frames left or right.
    WIDTH = 640 + 79
    # split the video vertically. The addition statement accounts for blackspace. Changing this number moves the quadrant frames left or right.
    HEIGHT = 480 + 60
